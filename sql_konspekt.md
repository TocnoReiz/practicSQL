...
Автоматически увеличивает значение колонки на 1 при каждой новой строке. Обычно используется вместе с `PRIMARY KEY`, чтобы не указывать id вручную.
...
```sql
INSERT INTO students (name) VALUES ('Дмитрий');
-- id подставится сам: 1, 2, 3...
```
...
Задаёт значение, которое подставится автоматически, если при добавлении строки его не указали.
...
CREATE TABLE students (
    ...
    name VARCHAR(50),
    age INT DEFAULT 18,
    city VARCHAR(50) DEFAULT 'Не указан'
);
```

```sql
INSERT INTO students (id, name) VALUES (1, 'Дмитрий');
-- age станет 18, city станет 'Не указан'
```
...
Показывает структуру таблицы: какие колонки, какие у них типы, ограничения (NOT NULL, PRIMARY KEY и т.д.).

```sql
DESCRIBE students;
```

Полезно, когда открыл чужую или старую таблицу и хочешь быстро понять, что в ней есть, не заглядывая в код создания.

---

## DROP (подробнее)

Полностью удаляет объект — таблицу, базу данных и т.д. В отличие от `DELETE` (удаляет строки) и `TRUNCATE` (очищает таблицу), `DROP` удаляет саму структуру безвозвратно.
...
Можно добавить проверку, чтобы не было ошибки, если объекта уже нет:
```sql
DROP TABLE IF EXISTS students;
```

⚠️ Действие необратимо — перед использованием стоит быть уверенным, что таблица/база точно не нужна.
...
- `DELETE` — удаляет строки, `TRUNCATE` — очищает всю таблицу, `DROP` — удаляет саму таблицу целиком
